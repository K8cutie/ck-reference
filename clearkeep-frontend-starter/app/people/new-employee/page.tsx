...
import { apiPost, setApiKey } from '../../../lib/api'; // ← correct path (3x ..)
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href="/people/employees" className="rounded border px-3 py-1 text-sm hover:bg-gray-100">
            ← Back to Employees
          </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <button onClick={saveKey} className="rounded bg-white text-black px-3 py-1 text-sm">
            Save
          </button>
        </div>
      </div>
      ...
      {created && (
        <div className="rounded border border-green-600 bg-green-500/10 p-3 text-sm">
      ...
      <div className="rounded-2xl border p-4 shadow-sm bg-white/5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <Row label="First name">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.first_name} onChange={(e) => setF({ ...f, first_name: e.target.value })}/>
            </Row>
            <Row label="Last name">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.last_name} onChange={(e) => setF({ ...f, last_name: e.target.value })}/>
            </Row>
            <Row label="Pay type">
            ...
            <Row label="Monthly rate">
              <input type="number" step="0.01" className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.monthly_rate} onChange={(e) => setF({ ...f, monthly_rate: e.target.value })}/>
            </Row>
            <Row label="Daily rate">
              <input type="number" step="0.01" className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.daily_rate} onChange={(e) => setF({ ...f, daily_rate: e.target.value })}/>
            </Row>
            <Row label="Hourly rate">
              <input type="number" step="0.01" className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.hourly_rate} onChange={(e) => setF({ ...f, hourly_rate: e.target.value })}/>
            </Row>

            <div className="text-sm font-semibold mt-6 mb-2">Government / Tax</div>
            <Row label="Tax status">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.tax_status} onChange={(e) => setF({ ...f, tax_status: e.target.value })}/>
            </Row>
            <Row label="SSS no">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.sss_no} onChange={(e) => setF({ ...f, sss_no: e.target.value })}/>
            </Row>
            <Row label="PhilHealth no">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.philhealth_no} onChange={(e) => setF({ ...f, philhealth_no: e.target.value })}/>
            </Row>
            <Row label="Pag-IBIG no">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.pagibig_no} onChange={(e) => setF({ ...f, pagibig_no: e.target.value })}/>
            </Row>
            <Row label="TIN">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.tin} onChange={(e) => setF({ ...f, tin: e.target.value })}/>
            </Row>
          </div>
          ...
          <div>
            ...
            <Row label="Hire date">
              <input type="date" className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.hire_date} onChange={(e) => setF({ ...f, hire_date: e.target.value })}/>
            </Row>
            <Row label="Termination date">
              <input type="date" className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.termination_date} onChange={(e) => setF({ ...f, termination_date: e.target.value })}/>
            </Row>

            <div className="text-sm font-semibold mt-6 mb-2">Contacts & Address</div>
            <Row label="Contact no">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.contact_no} onChange={(e) => setF({ ...f, contact_no: e.target.value })}/>
            </Row>
            <Row label="Email">
            ...
            <Row label="Address 1">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.address_line1} onChange={(e) => setF({ ...f, address_line1: e.target.value })}/>
            </Row>
            <Row label="Address 2">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.address_line2} onChange={(e) => setF({ ...f, address_line2: e.target.value })}/>
            </Row>
            <Row label="Barangay">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.barangay} onChange={(e) => setF({ ...f, barangay: e.target.value })}/>
            </Row>
            <Row label="City">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
            ...
            <Row label="Province">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.province} onChange={(e) => setF({ ...f, province: e.target.value })}/>
            </Row>
            <Row label="Postal code">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.postal_code} onChange={(e) => setF({ ...f, postal_code: e.target.value })}/>
            </Row>
            <Row label="Country">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.country} onChange={(e) => setF({ ...f, country: e.target.value })}/>
            </Row>

            <div className="text-sm font-semibold mt-6 mb-2">Emergency & Notes</div>
            <Row label="Emergency name">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.emergency_contact_name} onChange={(e) => setF({ ...f, emergency_contact_name: e.target.value })}/>
            </Row>
            <Row label="Emergency no">
              <input className="rounded border px-2 py-1 w-full bg-transparent"
                     value={f.emergency_contact_no} onChange={(e) => setF({ ...f, emergency_contact_no: e.target.value })}/>
            </Row>
            <Row label="Notes">
        ...
        <div className="mt-4 flex items-center gap-2">
          ...
          <Link href="/people/employees" className="rounded border px-3 py-2 text-sm hover:bg-gray-100">
            Cancel
          </Link>
        </div>
      </div>
